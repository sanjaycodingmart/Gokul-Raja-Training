...
const getAbout = (request, response) => {
  ...
  pool.query("SELECT email ,about FROM chatusers", (error, results) => {
    if (error) {
      throw error;
    }
    response.status(200).json(results.rows);
...
const createUser = (request, response) => {
  ...
  const status = "active";
  pool.query(
    "INSERT INTO chatusers (email,password,name,created_at,updated_at,status) VALUES ($1, $2 ,$3,$4,$5,$6)",
    [email, password, name, created_at, updated_at, status],
    (error, results) => {
      if (error) {
        throw error;
      }
