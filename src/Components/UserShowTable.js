...
function EntriedTable(props) {
    const [data, setData] = React.useState(() =>
    makeData(props.row, props.respond)
  );
  React.useEffect(() => {
   let dataRec= props.data;
    setData(dataRec);
  },[props.data]);
  ...
  const deleteData = () =>{
    let firebased = firebase.database().ref(props.user+"/files/");
    firebased.child(props.file).remove();
  ...
  return (
    ...
    <Fragment>
      ...
      <button onClick={deleteData} style={{ float: "right" }}>
        Delete Table
      </button>
